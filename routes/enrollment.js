  ...
  verifyUserToken(req.body.token, item => {
    if (!item || !item.isValid) {
      return res.status(200).json({
        error: true,
        message:
          'User session expired. Please log in again to proceed with enrollment.'
      })
    } else {
      ...
      enrollmentModel.find(
        { userId: item.user_id },
        (enrolledCourseErr, enrolledCourseDocs) => {
          if (!enrolledCourseErr) {
            courseModel.findOne(
              { _id: courseID, status: 'Listed' },
              (courseErr, course) => {
                if (courseErr) {
                  return res.status(200).json({
                    error: true,
                  ...
                  for (var i = 0; i < enrolledCourseDocs.length; i++) {
                    if (
                      enrolledCourseDocs[i].courseId == courseID &&
                      !enrolledCourseDocs[i].completed
                    ) {
                      courseProcess = true
                      break
                    } else if (
                    ...
                    course.prerequisites.forEach(prereq => {
                      if (
                        enrolledCourseDocs[i].courseId.toString() ==
                        prereq._id.toString()
                      ) {
                  ...
                  if (courseProcess) {
                    ...
                    return res.status(200).json({
                      error: true,
                      message:
                        'You have already completed this course. You can review it in your courses page'
                    })
                  } else {
                    let incompleteReqs = false
                    if (temp.length < course.prerequisites.length)
                      incompleteReqs = true
                    else {
                    ...
                    if (incompleteReqs) {
                      return res.status(200).json({
                        error: true,
                        message:
                          'You have not completed the required pre-requisites for this course.'
                      })
                    } else {
  ...
  verifyUserToken(req.body.token, item => {
    if (!item || !item.isValid) {
      return res.status(200).json({
        error: true,
        message:
          'User session expired. Please log in again to proceed with de-enrollment.'
      })
    } else {
      ...
      enrollmentModel.findOneAndDelete(
    ...
    verifyUserToken(req.body.token, item => {
      if (!item || !item.isValid) {
        ...
        enrollmentModel.findOne(
          { userId: item.user_id, courseId: courseID },
          (err, doc) => {
            if (err || !doc) {
              return res.status(200).json({
                error: true,
              ...
              courseModel.findOne({ _id: courseID }, (err, document) => {
                if (err || !document) {
                  return res.status(200).json({
                    error: true,
                    message:
                      'An unexpected error occurred while fetching course details. Please try again later.'
                  })
                } else {
                  return res.status(200).json({
                    error: false,
...
router.post('/getEnrollmentData', (req, res) => {
  if (!req.body.token) {
    return res.status(200).json({
      error: true,
      message: 'User token is required.'
        ...
        enrollmentModel.findOne(
          ...
          (err, doc) => {
            if (err || !doc) {
              ...
              return res.status(200).json({
                error: false,
                message: 'Enrollment course data found.',
                data: doc,
                course: document
              })
            }
